...
public class Utility {
	...
	public static WebElement highlightElement(WebDriver driver, WebElement element) {
		...
		js.executeScript("arguments[0].setAttrbute('style','background:yellow; border: 2px solid red')", element);
		
		waitForSeconds(1);
		
		js.executeScript("arguments[0].setAttrbute('style','background: ; border: 2px solid white')", element);
	...
	public static void captureScreenshot(WebDriver driver) {
		try 
		{
			FileHandler.copy(((TakesScreenshot)driver).getScreenshotAs(OutputType.FILE),new File("./Screenshots/MyScreenshot"+getCurrentDateTime()+".png"));
		} 
		catch (IOException e) 
		{
	...
	public static String  getCurrentDateTime() {
		String currentDateTime = new SimpleDateFormat("dd_MM_yyyy").format(new Date());
		return currentDateTime;
	}
	
	public static void getWebElementScreenshot(WebDriver driver,WebElement element) {
		try {
			FileHandler.copy(element.getScreenshotAs(OutputType.FILE), new File("./Element/element"+getCurrentDateTime()+".png"));
		} catch (IOException e) {
			System.out.println("There is some problem with screenshot");
		}
	}
